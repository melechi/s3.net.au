	...
	<% control Member %>
		<div id="UserProfile">
			...
			<% if Avatar %>
				...
				<% end_control %> </p></div>
			<% else %>
				<div><label class="left">Avatar:</label> <p><img class="userAvatar" src="themes/itheme_forum/images/forummember_holder.gif" width="80" alt="<% if Nickname %>$Nickname<% else %>Anon<% end_if %>&#39;s avatar" /></p></div>
			<% end_if %>
		</div>
	<% end_control %>
	<% if LatestPosts %>
